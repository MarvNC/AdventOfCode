...
let wire1 = points(input[0]);
let wire2 = points(input[1]);

let all = new Map();
wire1.forEach((elem) => {
  all.set(elem.pos.toString(), elem.steps);
});
let intersect = wire2.filter((elem) => all.has(elem.pos.toString()));

console.log(intersect);

// let ans = intersect.reduce(
//   (prev, curr) => Math.min(Math.abs(curr.pos[0]) + Math.abs(curr.pos[1]), prev),
//   Number.MAX_SAFE_INTEGER
// );
let ans = intersect.reduce(
  (prev, curr) => Math.min(all.get(curr.pos.toString()) + curr.steps, prev),
  Number.MAX_SAFE_INTEGER
);
console.log(ans);

function points(wire) {
  let positions = [];
  let currentPos = [0, 0];
  let steps = 0;
  wire = wire.split(',');
  wire.map((move) => {
    let i = 0;
    while (i < parseInt(move.substring(1))) {
      ...
      steps++;
      positions.push({ pos: [...currentPos], steps: steps });
      i++;
    }
  });
  return positions;
