let input = `16-18 h: hhhhhhhhhhhhhhhhhh
...
input = input.map((elem) => {
  ...
  pass.min = range[0];
  pass.max = range[1];
  pass.letter = arr[1].charAt(0);
  pass.password = arr[2];
  return pass;
});

let count = input.filter((elem) => isValid(elem)).length;
console.log(count);

function isValid(pass) {
  let first = pass.password.charAt(pass.min - 1) == pass.letter;
  let last = pass.password.charAt(pass.max - 1) == pass.letter;
  // console.log(pass, first != last);
  return first != last;
}