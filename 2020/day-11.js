...
let input = `LLLLLLLL.LLL.LLLLLLLLLLLLL.LL.LLLLLL.LL.LLLLLLLLLLLLLL.LLLLLLL.LLLLLLLLLLLLLLLLLLL.LLLLLLLL
  ...
  return dupe.map((row, rowIndex) => {
    ...
    return row
      .map((char, col) => {
        ...
        if (char == EMPTY && neighbors == 0) return OCCUPIED;
        if (char == OCCUPIED && neighbors >= (part == 1 ? 4 : 5)) {
          return EMPTY;
        }
        return char;
      })
...
function getNeighbors(grid, row, col, part) {
  if (part == 1) {
    return [
      getAt(grid, row - 1, col),
      getAt(grid, row - 1, col - 1),
      getAt(grid, row - 1, col + 1),
      getAt(grid, row + 1, col),
      getAt(grid, row + 1, col - 1),
      getAt(grid, row + 1, col + 1),
      getAt(grid, row, col + 1),
      getAt(grid, row, col - 1),
  ...
  return directions
    .map((direction) => {
      let i = row + direction[0],
        j = col + direction[1];
      while (i < grid.length && i >= 0 && j < width && j >= 0) {
        let spot = getAt(grid, i, j);

        if (spot == OCCUPIED || spot == EMPTY) return spot;
...
function getAt(grid, row, col) {
  if (row >= 0 && row < grid.length && col >= 0 && col < width) {
    return grid[row][col];
  }
  return EMPTY;
}

function countPeople(grid) {
  return grid.reduce(
    (prev, curr) => prev + curr.split('').filter((char) => char == OCCUPIED).length,
    0
  );
}
...
function equillibrium(grid, part) {
  let newGrid = grid;
  let lastGrid;
  do {
    lastGrid = newGrid;
    newGrid = next(lastGrid, part);
  } while (!compare(newGrid, lastGrid));
  return lastGrid;
}

console.log(countPeople(equillibrium(input, 1)));
console.log(countPeople(equillibrium(input, 2)));