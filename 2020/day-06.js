...
// part 1
console.log(input.reduce((prev, group) => prev + groupStats(group).questions, 0));

// part 2
console.log(input.reduce((prev, group) => prev + groupStats(group).unanimous, 0));

function groupStats(answers) {
  stats = getAnswers(answers);
  let people = answers.split('\n').length;
  let unanimous = Array.from(stats).filter((question) => question[1] == people);

  return {
    questions: stats.size,
    people: people,
...
function getAnswers(answers) {
  ...
  answers.forEach((person) => {
    person.split('').forEach((question) => {
      if (results.has(question)) results.set(question, results.get(question) + 1);
      else results.set(question, 1);
    });
  });
  return results;
}
