...
passports = passports.map((passport) => passport.replace(/\n/g, ' '));

let valid = passports.filter((pass) => {
  pass = fieldParse(pass);
  return isValid(pass);
});
...
function isValid(pass) {
  let { birth, issue, expiry, height, hair, eye, passID } = pass;
  if (birth && issue && expiry && height && hair && eye && passID) {
    ...
    let unit = height.match(/[a-z]+/);
    height = height.match(/\d+/);
    if (!unit || !height) return;
    height = height[0];
    switch (unit[0]) {
      case 'cm':
        if (height < 150 || height > 193) return;
        break;
      case 'in':
        if (height < 59 || height > 76) return;
        break;
    }
    if (hair.length != 7) return;
    if (!hair.match(/#[\da-f]{6}/)) return;
    if (eye.length != 3) return;
    if (!eye.match(/(amb)|(blu)|(brn)|(gry)|(grn)|(hzl)|(oth)/)) return;
    if (passID.length != 9 || parseInt(passID) == NaN) return;
    return true;
  }
}

function fieldParse(input) {
  input = input.split(' ');
  let fields = {};
  input.forEach((trait) => {
    trait = trait.split(':');
    switch (trait[0]) {
      case 'byr':
        fields.birth = trait[1];
        break;
      case 'iyr':
        fields.issue = trait[1];
        break;
      case 'eyr':
        fields.expiry = trait[1];
        break;
      case 'hgt':
        fields.height = trait[1];
        break;
      case 'hcl':
        fields.hair = trait[1];
        break;
